...
"""
Returns a number of feature vectors from a matrix
...
def generate_feature_vectors_from_samples(matrix, nsamples, period,
                                          state=None,
                                          remove_redundant=True,
                                          cols_to_ignore=None):
    """
    Data is passed in as a numpy array statistical features are extracted from the data
    ...
    Parameters:
        matrix (numpy.ndarray): numpy array containing the records
    ...
    # removed state from headers[:-1] (see original method for comparison)
    # it used to delete the last feature name (the state)
    feat_names = ["lag1_" + s for s in headers] + headers
    ...
    if remove_redundant:
        ...
        # np.delete will error with axis=1 for a 1D array, requires axis=None to delete a single element
        # axis = 1 if ret.shape.__len__() > 1 else None
...
def generate_feature_vectors_from_samples_csv(file_path, nsamples, period,
                                              state=None,
                                              remove_redundant=True,
                                              cols_to_ignore=None):
    """
